...
cursor.execute('''CREATE TABLE IF NOT EXISTS weather (city TEXT, temp Numeric, humidity NUMERIC, pressure NUMERIC, wind NUMERIC)''');
cursor.execute('''INSERT INTO weather values (?, ?, ?, ?, ?)''', (city, temp, humid, press, current["speed"]));
...
for row in cursor:
    print(row['city'], row['temp'], row['humidity'], row['pressure'], row['wind']);
